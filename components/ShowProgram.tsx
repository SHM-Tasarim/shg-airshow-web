...
const ShowProgram: React.FC<ShowProgramProps> = ({ lang, onNavigate }) => {
  ...
  const translations = {
    ...
    day1: lang === 'TR' ? '18 EYLÜL 2027' : '18 SEPTEMBER 2027',
    day2: lang === 'TR' ? '19 EYLÜL 2027' : '19 SEPTEMBER 2027',
    soon: lang === 'TR' ? 'YAKINDA' : 'COMING SOON',
  };

  const programs = [
    { label: translations.day1 },
    { label: translations.day2 },
  ];
      ...
      <div className="max-w-5xl mx-auto text-center px-4">
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-4 sm:gap-6">
          {programs.map((program, idx) => (
            <div
              key={idx}
              className="flex flex-col items-center gap-2 sm:gap-4 bg-white dark:bg-gray-900/40 border-2 border-gray-200 dark:border-gray-800 rounded-xl sm:rounded-2xl p-4 sm:p-8 w-full sm:w-auto"
            >
              <span className="material-icons text-4xl sm:text-6xl text-gray-400">
                event_note
              </span>
